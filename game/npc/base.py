...
class NPC(Character):
    ...
    def _find_next_step(self, target_x, target_y, game_map, max_search_distance=50):
        ...
        # ВАЖНО: порядок имеет значение! Сначала прямые (стоимость 1.0), потом диагональные (стоимость ~1.414)
        # Это заставляет BFS предпочитать прямые пути при равной длине маршрута
        directions = [
            (1, 0),  (-1, 0),  (0, 1),  (0, -1),  # Прямые направления (вправо, влево, вниз, вверх)
