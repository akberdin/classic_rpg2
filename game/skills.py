...
class Mining(Skill):
    ...
    def use(self, user, target=None):
        ...
        # Шанс успеха растет с рангом (50% + 10% за ранг)
        success_chance = 0.5 + (self.rank - 1) * 0.1

        if random.random() < success_chance:
            # Количество ресурсов растет с рангом
            quantity = 1 + (self.rank // 2)
            result['success'] = True
            result['quantity'] = quantity
            result['message'] = f"{user.name} добыл руду! Количество: {quantity}"
        else:
            result['success'] = False
            result['message'] = f"{user.name} не смог добыть руду"
...
class Lumberjacking(Skill):
    ...
    def use(self, user, target=None):
        ...
        if random.random() < success_chance:
            ...
            result['success'] = True
            result['quantity'] = quantity
            result['message'] = f"{user.name} срубил дерево! Количество древесины: {quantity}"
        else:
            result['success'] = False
            result['message'] = f"{user.name} не смог срубить дерево"
